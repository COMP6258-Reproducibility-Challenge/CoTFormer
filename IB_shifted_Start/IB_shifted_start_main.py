...
def main(args):
    ...
    train_dataset = load_counting_split(data_root, spec.name, "train")
    val_dataset = load_counting_split(data_root, spec.name, "val")
    eval_datasets = {
        split: val_dataset if split == "val" else load_counting_split(data_root, spec.name, split)
        for split in args.ib_eval_splits
    }

    print_master(distributed_backend, f"Loading shifted-start task '{spec.name}' from {data_root}")
    print_master(distributed_backend, f"Num train examples: {len(train_dataset)}")
    print_master(distributed_backend, f"Num val examples: {len(val_dataset)}")
